  ...
  <div>
    <section>
      <v-container class="tw-grid tw-grid-cols-2 tw-gap-4">
        <s-card dark>
          <s-card-title class="tw-pb-0">
            成交价格
          </s-card-title>
          <s-chart autoresize theme="dark" :options="ChartHistoryPriceTrend(school.history)" style="height: 200px" />
        </s-card>
        <s-card>
          <s-card-title class="tw-pb-0">
            成交户型
          </s-card-title>
          <s-chart autoresize theme="dark" :options="ChartHistoryRoomBreakdown(school.history)" style="height: 200px" />
        </s-card>
      </v-container>
    </section>
      ...
      <v-container>
        ...
        >
          <template #[`item.houseSize`]="{ value }">
            {{ _.round(value, 1) }}<span class="tw-text-xs"> m<sup>2</sup></span>
          </template>
          <template #[`item.blurredFloor`]="{ item }">
            {{ item.blurredFloor }}<span class="tw-text-xs"> (共{{ item.totalFloor }}层)</span>
          </template>
...
<script>
import ChartHistoryPriceTrend from '~/components/organisms/ChartHistoryPriceTrend'
import ChartHistoryRoomBreakdown from '~/components/organisms/ChartHistoryRoomBreakdown'
export default {
  name: 'TheSchoolDetailErshouHistory',
  props: {
    school: {
      type: Object,
      required: true
    ...
    series () {
      ...
      for (const groupName in groups) {
        series.price[0].data.push({
          x: groupName,
          y: this.$_.round(this.$_.meanBy(groups[groupName], 'unitPrice'), 0) + '元'
        })
  ...
  watch: {
    'project.name': {
      async handler () {
        ...
        this.loading = true
        try {
          this.items = await this.$api.unit.get('project/中远两湾城')
        } catch (e) {
          console.log(e)
        }
        this.loading = false
  ...
  methods: {
    ChartHistoryPriceTrend,
    ChartHistoryRoomBreakdown,
